...
export class UtxoSet extends EventTargetImpl {
   ...
   * @param utxos Array of UTXOs from hoosat API.
  ...
  updateUtxoBalance(): void {
    ...
    [...this.utxos.pending.values()].forEach((utxo) => {
      if (blueScore - utxo.blockDaaScore < (utxo.isCoinbase ? COINBASE_CFM_COUNT : CONFIRMATION_COUNT)) return;
  ...
  collectUtxos(maxCount: number = 10000): {
    ...
    let mass = 0;
    for (const utxo of list) {
      if (utxos.length >= maxCount || mass + utxo.mass >= maxMass) break;
      utxoIds.push(utxo.id);
      utxos.push(utxo);
      totalVal += utxo.satoshis;
      mass += utxo.mass;
  ...
  async syncAddressesUtxos(addresses: string[]) {
    const newAddresses = addresses
      .map((address) => {
        if (this.addressesUtxoSyncStatuses.has(address)) return;
    ...
    if (!newAddresses.length || (this.wallet.syncInProggress && !this.wallet.options.disableAddressDerivation)) return;

    await this.wallet.findUtxos(newAddresses);

    if (!this.wallet.syncOnce) await this.utxoSubscribe();
  }
  ...
  async utxoSubscribe(): Promise<string[]> {
    ...
    let utxoChangedRes = await this.wallet.api
      .subscribeUtxosChanged(addresses, this.onUtxosChanged.bind(this))
