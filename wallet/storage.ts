...
class LSDB extends DBInterface {
  ...
  backup(): void {
    ...
    if (cache) this.LS.setItem("hoosat-cache-" + ts, cache);
  }
...
if (IS_NODE) {
  ...
  class FileDB extends DBInterface {
    ...
    getHomeFolder() {
      ...
      if (process.platform == "darwin") return path.join(HOME, "/Library/Application Support/Hoosat/");
    ...
    getWallet(): string | false {
      if (fs.existsSync(this.walletFile)) return fs.readFileSync(this.walletFile) + "";
      return false;
    }
    ...
    getCache(): string | undefined {
      if (fs.existsSync(this.cacheFile)) return fs.readFileSync(this.cacheFile) + "";
      return undefined;
    }
  }
...
export class Storage {
  ...
  constructor(opt: StorageOpts = {}) {
    ...
    if (opt.logLevel) this.setLogLevel(opt.logLevel);
