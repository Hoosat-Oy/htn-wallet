...
const API_BASE = "https://api.hoosat.org/";
...
export class TXStore {
  ...
  add(tx: TXStoreItem, skipSave = false) {
    ...
    if (this.store.has(tx.id)) return false;
    this.store.set(tx.id, tx);
    this.emitTx(tx);
    if (this.store.size > TXStore.MAX) this.store = new Map([...this.store.entries()].slice(-TXStore.MAX));
    if (!skipSave) this.save(tx);
    return true;
  }
  ...
  async addAddressUTXOs(address: string, utxos: Api.Utxo[], ts?: number) {
    if (!utxos.length || this.wallet.addressManager.isOurChange(address)) return;
  ...
  async updateTransactionTimeImpl(
    txIdList: string[] | null = null,
  ...
  async getDBEntries(
  ...
  ): Promise<{ list: TXStoreItem[]; txWithMissingVersion: string[] }> {
    if (!this.idb) {
      return {
        list: [],
    ...
    let entries =
      (await this.idb.entries().catch((err) => {
