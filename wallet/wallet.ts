...
import { HoosatAPI, ApiError } from "./api";
...
const SompiPerHoosat = 100_000_000;

// MaxSompi is the maximum transaction amount allowed in sompi.
const MaxSompi = 21_000_000 * SompiPerHoosat;

export { hoosatcore, COMPOUND_UTXO_MAX_COUNT, CONFIRMATION_COUNT, COINBASE_CFM_COUNT };
...
class Wallet extends EventTargetImpl {
  ...
  //hoosatcore.Transaction.EstimatedStandaloneMassWithoutInputs;
  ...
  constructor(privKey: string, seedPhrase: string, networkOptions: NetworkOptions, options: WalletOptions = {}) {
    ...
    if (networkOptions.rpc) this.api.setRPC(networkOptions.rpc);
    ...
    if (this.options.disableAddressDerivation) this.addressManager.receiveAddress.next();
  ...
  async onApiConnect() {
    ...
    if (this.syncSignal && connected !== undefined) {
      //if sync was called
  ...
  async sync(syncOnce: boolean | undefined = undefined) {
    ...
    await this.initBlueScoreSync(syncOnce).catch((e) => {
    ...
    if (this.options.disableAddressDerivation) {
      ...
      await this.addressDiscovery(this.options.addressDiscoveryExtent).catch((e) => {
        this.logger.info("addressDiscovery:error", e);
      });
    }

    this.syncInProggress = false;
    if (!syncOnce) await this.utxoSet.utxoSubscribe();

    const ts1 = Date.now();
    const delta = ((ts1 - ts0) / 1000).toFixed(1);
    this.logger.info(`sync ... ${this.utxoSet.count} UTXO entries found`);
    this.logger.info(
      `sync ... indexed ${this.addressManager.receiveAddress.counter} receive and ${this.addressManager.changeAddress.counter} change addresses`
    );
    ...
    if (!this.utxoSet.clearMissing()) this.updateDebugInfo();
  }
  ...
  async initBlueScoreSync(once: boolean = false) {
    if (this.syncVirtualSelectedParentBlueScoreStarted) return;
    ...
    this.api
      .subscribeVirtualDaaScoreChanged((result) => {
  ...
  initAddressManager() {
    if (this.addressManagerInitialized) return;
    ...
    this.addressManager.on("new-address", (detail) => {
      ...
      if (this.options.skipSyncBalance) return;
  ...
  emitBalance(): void {
    ...
    if (available == _available && pending == _pending) return;
  ...
  updateDebugInfo() {
    ...
    this.utxoSet.inUse.map((utxoId) => {
      inUseUTXOs.satoshis +=
        confirmed.get(utxoId)?.satoshis || pending.get(utxoId)?.satoshis || used.get(utxoId)?.satoshis || 0;
    });
  ...
  emitAddress() {
    ...
    if (receive == _receive && change == _change) return;
  ...
  async scanMoreAddresses(count = 100, debug = false, receiveStart = -1, changeStart = -1): Promise<ScaneMoreResult> {
    if (this.syncInProggress) return { error: "Sync in progress", code: "SYNC-IN-PROGRESS" };

    if (receiveStart < 0) receiveStart = this.addressManager.receiveAddress.counter;

    if (changeStart < 0) changeStart = this.addressManager.changeAddress.counter;

    this.syncInProggress = true;
    this.emit("scan-more-addresses-started", { receiveStart, changeStart });
    let error = false;
    let res = await this.addressDiscovery(
      this.options.addressDiscoveryExtent,
      debug,
      receiveStart,
      changeStart,
      count
    ...
    this.syncInProggress = false;
    if (!this.syncOnce) this.utxoSet.utxoSubscribe();
    this.emit("scan-more-addresses-ended", { error });

    if (error) return { error, code: "ADDRESS-DISCOVERY" };
  ...
  async addressDiscovery(
    ...
    const doDiscovery = async (n: number, deriveType: "receive" | "change", offset: number): Promise<number> => {
      ...
      this.logger.verbose(
        `${deriveType}: address data for derived indices ${derivedAddresses[0].index}..${
          derivedAddresses[derivedAddresses.length - 1].index
        }`
      );
      ...
      if (!debugInfo) debugInfo = txID2Info;
    ...
    if (!this.syncOnce && !this.syncInProggress) await this.utxoSet.utxoSubscribe();
   ...
   * @param obj
   * @param obj.toAddr To address in cashaddr format (e.g. hoosattest:qq0d6h0prjm5mpdld5pncst3adu0yam6xch4tr69k2)
  ...
  composeTx({
    ...
    try {
      const tx: hoosatcore.Transaction = new hoosatcore.Transaction()
        .from(utxos)
        .to(toAddr, amount)
        .setVersion(0)
        .fee(fee)
        .change(changeAddr);
      if (!skipSign) tx.sign(privKeys, hoosatcore.crypto.Signature.SIGHASH_ALL, "schnorr");
      ...
      // !!! FIXME
      if (!changeAddrOverride) this.addressManager.changeAddress.reverse();
      throw e;
    }
  }
  ...
  /*
	validateAddress(addr:string):boolean{
		let address = new hoosatcore.Address(addr);
  ...
  async composeTxAndNetworkFeeInfo(txParamsArg: TxSend): Promise<TxInfo> {
    ...
    if (!txParamsArg.fee) txParamsArg.fee = 0;
    this.logger.info(`tx ... sending to ${txParamsArg.toAddr}`);
    this.logger.info(
      `tx ... amount: ${HTN(txParamsArg.amount)} user fee: ${HTN(txParamsArg.fee)} max data fee: ${HTN(
        txParamsArg.networkFeeMax || 0
      )}`
    );
    ...
    if (networkFeeMax && dataFee > networkFeeMax) {
      throw new Error(
        `Fee max is ${networkFeeMax} but the minimum fee required for this transaction is ${HTN(dataFee)} HTN`
      );
    }
    ...
    if (calculateNetworkFee) {
      do {
        ...
        if (data.utxos.length != utxoLen) this.logger.verbose(`tx ... aggregating: ${data.utxos.length} UTXOs`);
      ...
      if (networkFeeMax && txParams.fee > networkFeeMax)
        throw new Error(
          `Maximum network fee exceeded; need: ${HTN(dataFee)} HTN maximum is: ${HTN(networkFeeMax)} HTN`
        );
  ...
  async buildTransaction(txParamsArg: TxSend, debug = false): Promise<BuildTxResult> {
    ...
    const ts_0 = Date.now();
    tx.sign(privKeys, hoosatcore.crypto.Signature.SIGHASH_ALL, "schnorr");
    ...
    if (debug || this.loggerLevel > 0) this.logger.debug("composeTx:tx", "txSize:", txSize);

    const ts_3 = Date.now();
    const inputs: RPC.TransactionInput[] = tx.inputs.map((input: hoosatcore.Transaction.Input) => {
      if (debug || this.loggerLevel > 0) {
    ...
    const ts_4 = Date.now();
    const outputs: RPC.TransactionOutput[] = tx.outputs.map((output: hoosatcore.Transaction.Output) => {
      return {
    ...
    //@ ts-ignore
    //const payloadHash = hoosatcore.crypto.Hash.sha256sha256(Buffer.from(payloadStr));
  ...
  async submitTransaction(txParamsArg: TxSend, debug = false): Promise<TxResp | null> {
    ...
    const { rpcTX, utxoIds, amount, toAddr, note, utxos } = await this.buildTransaction(txParamsArg, debug);
    ...
    try {
      ...
      if (!txid) {
        if (reverseChangeAddress) this.addressManager.changeAddress.reverse();
  ...
  async compoundUTXOs(txCompoundOptions: TxCompoundOptions = {}, debug = false): Promise<TxResp | null> {
    ...
    try {
      ...
      if (!res?.txid) this.addressManager.changeAddress.reverse();
      return res;
    } catch (e) {
  ...
  setLogLevel(level: string) {
    ...
    hoosatcore.setDebugLevel(level ? 1 : 0);
  }
}
