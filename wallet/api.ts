...
export class ApiError {
  ...
  constructor(message: string, debugInfo: any = null) {
    ...
    if (!Error.captureStackTrace) this.stack = (new Error(message).stack + "").split("↵").join("\n");
    else Error.captureStackTrace(this, ApiError);
  }
...
class HoosatAPI extends EventTargetImpl {
  rpc?: IRPC;
  ...
  async getVirtualSelectedParentBlueScore(): Promise<{ blueScore: number }> {
    if (!this.rpc) return missingRPCProviderError();

    const response = await this.rpc.getVirtualSelectedParentBlueScore().catch((e) => {
      throw new ApiError(`API connection error. ${e}`);
    });

    if (response.error) throw new ApiError(`API error (${response.error.errorCode}): ${response.error.message}`);
  ...
  async getVirtualDaaScore(): Promise<{ virtualDaaScore: number }> {
    ...
    const response = await this.rpc.getBlockDagInfo().catch((e) => {
      throw new ApiError(`API connection error. ${e}`);
    });
  ...
  async subscribeVirtualSelectedParentBlueScoreChanged(
    callback: RPC.callback<RPC.VirtualSelectedParentBlueScoreChangedNotification>
  ) {
    if (!this.rpc) return missingRPCProviderError();

    const response = await this.rpc.subscribeVirtualSelectedParentBlueScoreChanged(callback).catch((e) => {
      throw new ApiError(`API connection error. ${e}`);
    });
  ...
  async subscribeVirtualDaaScoreChanged(callback: RPC.callback<RPC.VirtualDaaScoreChangedNotification>) {
    ...
    const response = await this.rpc.subscribeVirtualDaaScoreChanged(callback).catch((e) => {
      throw new ApiError(`API connection error. ${e}`);
    });
  ...
  async subscribeUtxosChanged(
    addresses: string[],
    callback: (added: Map<string, Api.Utxo[]>, removed: Map<string, RPC.Outpoint[]>) => void
  ) {
    if (!this.rpc) return missingRPCProviderError();
    ...
    if (_utxosChangedSubUid) this.rpc.unSubscribeUtxosChanged(_utxosChangedSubUid);
  ...
  async submitTransaction(tx: RPC.SubmitTransactionRequest): Promise<string> {
    ...
    if (!response.error) response.error = { message: "Api error. Please try again later. (ERROR: SUBMIT-TX:100)" };
    if (!response.error.errorCode) response.error.errorCode = 100;
