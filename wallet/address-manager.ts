...
// @ts-ignore
const secp256k1 = hoosatcore.secp256k1; //require('secp256k1-wasm');
...
export class AddressManager extends EventTargetImpl {
  constructor(HDWallet: hoosatcore.HDPrivateKey, network: Network) {
    super();
    this.HDWallet = HDWallet;
    this.network = network;
  }

  private HDWallet: hoosatcore.HDPrivateKey;

  network: Network;

  get all(): Record<string, hoosatcore.PrivateKey> {
    return {
  ...
  get shouldFetch(): string[] {
    const receive = Object.entries(this.receiveAddress.atIndex)
      .filter((record: [string, string]) => parseInt(record[0], 10) <= this.receiveAddress.counter - 1)
  ...
  receiveAddress: {
    ...
    current: {
      address: string;
      privateKey: hoosatcore.PrivateKey;
    };
    keypairs: Record<string, hoosatcore.PrivateKey>;
  ...
  private deriveAddress(
    ...
    //let address1 = new hoosatcore.PublicKey(publicKeys.pubkey, {network:this.network}).toAddress().toString();
    ...
    let address = hoosatcore.Address.fromPublicKeyBuffer(xonly, this.network).toString();
